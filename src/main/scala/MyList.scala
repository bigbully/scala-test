...
object MyList extends App{
  ...
  /*List.dropWhile1(List(1, 2, 3), (x:Int) => x > 1)*/
/*  println(List.foldRight(List(1,2,3), Nil:List[Int])(Cons(_,_)))*/
  /*println(List.reverse(List(1,2,3)))concat*/

  /*println(List.concat(List(List(1,2), List(3,4), List(5, 6))))*/
  /*println(List.add2(List(1,2, 3,4,5, 6 )))*/
/*  println(List.map(List(1.0,2.0, 3.1,4.2,5.3, 6.4 ))(_ + 1))*/
  /*println(List.zipGeneric(List("1", "2"), List("2", "3"))(_ + _))*/
  println(List.hasSubsequence2(List(1,2,3, 4), List( 1, 3)))
...
*/
  ...
  */
  ...
  def add2(l:List[Int]) :List[Int] = {
    foldLeft(l, Nil:List[Int])((newList, x) => Cons(x + 1, newList))
  }

  def add1(l: List[Int]): List[Int] =
    foldRight(l, Nil:List[Int])((h,t) => Cons(h+1,t))

  def transferToString(l:List[Double] ):List[String] = {
    foldRight(l, Nil:List[String]) ((head, l) => Cons( head.toString, l))
  }
  ...
  def filter[A](l:List[A])(f: A => Boolean):List[A] = {
    foldRight(l, Nil:List[A]){(head, tail) =>
      if (f(head)) {
        Cons(head, tail)
      }else {
        tail
  ...
  def zipGeneric[A](l1:List[A], l2:List[A])(f: (A, A)=> A):List[A] = {
    (l1, l2) match {
      case (Nil, _) => l2
      case (_, Nil) => l1
      case (Cons(h1, t1), Cons(h2, t2)) => Cons(f(h1 ,h2), zipGeneric(t1, t2)(f))
    }
  }

  @tailrec def hasSubsequence2[A](l:List[A], sub:List[A]):Boolean = {
    (l, sub) match {
      case (Nil, Nil) => true
      case (Nil, _) => false
      case (_, Nil) => true
      case (Cons(head, tail), Cons(subHead, subTail)) => {
        if (head == subHead) {
          hasSubsequence2(tail, subTail)
        }else {
          hasSubsequence2(tail, sub)
        }
      }
    }
  }
